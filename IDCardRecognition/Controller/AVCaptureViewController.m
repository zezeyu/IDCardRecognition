...
///反面检测区域
@property (nonatomic,assign) CGRect TailPathFrame;
...
- (void)viewDidLoad {
    ...
    if(_iSProsAndCons){
        ...
        CADTailView *TailView = [[CADTailView alloc] initWithFrame:self.view.frame];
        self.faceDetectionFrame = TailView.TailPathRect;
        [self.view addSubview:TailView];
        [TailView.ClickBut addTarget:self action:@selector(ClickButAction:) forControlEvents:UIControlEventTouchUpInside];
    }
...
#pragma mark - 确定点击
-(void)ClickButAction:(UIButton *)sender{
    self.isClick = YES;
}
