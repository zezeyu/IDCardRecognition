...
//  Copyright © 2018年 zhongfeng. All rights reserved.
//
...
@interface CADTailView () {
    CAShapeLayer *_IDCardScanningWindowLayer;
    NSTimer *_timer;
}
@end

@implementation CADTailView
...
-(void)addScaningWindow {
    ...
    self.TailPathRect = (CGRect){CGRectGetMaxX(rect) - facePathWidth - 35,CGRectGetMaxY(rect) - facePathHeight - 25,facePathWidth,facePathHeight};
    
    // 提示标签
    CGPoint center = self.center;
    center.x = CGRectGetMaxX(_IDCardScanningWindowLayer.frame) + 20;
    [self addTipLabelWithText:@"将身份证国徽面置于此区域内，对准，扫描" center:center];
...
-(UIButton *)ClickBut{
    if(!_ClickBut){
        ...
        _ClickBut.frame = CGRectMake(0, 0, _IDCardScanningWindowLayer.frame.size.height, _IDCardScanningWindowLayer.frame.size.width);
        _ClickBut.center = self.center;
        [_ClickBut setTitle:@"确定点击" forState:UIControlStateNormal];
        _ClickBut.transform = CGAffineTransformMakeRotation(M_PI * 0.5);
    }
    return _ClickBut;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    // 人像提示框
//    UIBezierPath *facePath = [UIBezierPath bezierPathWithRect:_TailPathRect];
